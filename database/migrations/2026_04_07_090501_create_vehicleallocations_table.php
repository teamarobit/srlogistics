...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('vehicleallocations', function (Blueprint $table) {
            ...
            $table->enum('type', ['Customer', 'Driver'])->nullable();
            $table->unsignedBigInteger('vehicle_id')->comment('FK → vehicles.id');
            $table->enum('change_vehicle', ['Yes', 'No'])->nullable();
            $table->longText('vehicle_change_reason')->nullable();
            $table->double('km_allowed')->default(0);
            $table->double('fixed_amount')->default(0);
            $table->double('extra_amount_per_km')->default(0);
