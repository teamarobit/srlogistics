/* =====================================================================
   SR Logistics — Vehicle Vendor Module V2 (Gate 2 — live wiring)
   Module-specific add-on to customer.js (loaded alongside it).
   External JS only (SD-1). customer.js drives the vendor form submit
   (#cv2CustomerForm / #cv2EditForm), intl-tel, Select2, state->city,
   status->blacklist, contact-person repeater, document + activity forms
   and validation-error display. This file adds:
     ...
     - GST Treatment conditional (show GST Number only when Registered)
     - List page toggle-status + delete (SD-3 $.ajax, SD-7 Toast)
     - Documents page delete
   Vehicle & Route sub-pages are READ-ONLY this Gate (no CRUD here).
   ===================================================================== */
$(function () {
    'use strict';
    ...
    /* ================= GST Treatment conditional (show GST Number when Registered) ===== */
    function cv2SyncGst() {
        var val = $('#cv2GstTreatment').val();
        $('[data-when="gst-registered"]').toggle(val === 'Registered');
    }
    if ($('#cv2GstTreatment').length) {
        $('#cv2GstTreatment').on('change', cv2SyncGst);
        cv2SyncGst();
    }

    /* ================= List page — toggle status ================= */
    $(document).on('click', '.cv2-toggle-vehicle', function () {
        var id = $(this).data('id');
        $.ajax({
            url: '/contacts/v2/vehicle-vendors/' + id + '/toggle-status',
            method: 'POST',
    ...
    $(document).on('click', '.cv2-del-vehicle', function () {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Delete this vehicle vendor?', text: 'This cannot be undone.', icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#ea0027', confirmButtonText: 'Delete'
        }).then(function (r) {
            if (!r.isConfirmed) { return; }
            $.ajax({
                url: '/contacts/v2/vehicle-vendors/' + id,
                method: 'POST',
                data: { _method: 'DELETE' },
                success: function (res) {
    ...
    /* ================= Documents page — delete ================= */
    $(document).on('click', '.cv2-del-vehicle-doc', function () {
        var id = $(this).data('id');
        Swal.fire({
            ...
            if (!r.isConfirmed) { return; }
            $.post('/contacts/v2/vehicle-vendors/attachment/delete', { id: id }, function (res) {
                Toast.fire({ icon: 'success', title: res.message || 'Deleted.' });
                setTimeout(function () { window.location.reload(); }, 800);
            }).fail(failToast);
        });
