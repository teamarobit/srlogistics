@if($vehicles->count())
    ...
    <tr>
        ...
        <td><span class="value">{{ $vehicle->vehicle_no ?? '' }}</span></td>
        <td>
        ...
        <td><span class="value">—</span></td>
        <td><span class="value">—</span></td>
        <td><span class="value">{{ $vehicle->groupTracking->managed_by_employee ?? '—' }}</span></td>
    ...
    <tr>
        <td colspan="11">
            <div class="d-flex justify-content-end mt-2">{!! $vehicles->links() !!}</div>
        </td>
    </tr>
@else
    <tr><td colspan="11" class="text-center text-muted">No vehicles found</td></tr>
@endif
